...
export default async (req: NextApiRequest, res: NextApiResponse<Data>) => {
  ...
  let magicSecretKey: string = process.env.MAGIC_SECRET_KEY
...
const createCustomFirebaseToken = async (uid: string): Promise<string> => {
  ...
  if (!process.env.FIREBASE_PRIVATE_KEY)
    throw new Error(
      "Error: Firebase private key not provided. Unable to create custom database token."
    );
  if (!process.env.FIREBASE_CLIENT_EMAIL)
    throw new Error(
      "Error: Firebase client email not provided. Unable to create custom database token."
    );
  // format firebase private key.. unless deployed on Vercel
  const isProduction: boolean = process.env.IS_PRODUCTION == "false";
  let keyToUse = isProduction
