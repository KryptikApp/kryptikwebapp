...
const Gallery: NextPage = () => {
  ...
  const [activeCategoryNftList, setActiveCategoryNftList] = useState<
    INFTMetadata[]
  >([]);
  const [activeCategoryNetworkDb, setActiveCategoryNetworkDb] =
    useState<NetworkDb | null>(defaultNetworkDb);
  ...
  const fetchNFTDataKryptik = async function () {
    ...
    let solanaAddress: string = await getAddressForNetworkDb(
      kryptikWallet,
      solanaNetworkDb
    );
    let newNftMetadataList: INFTMetadata[] = [];
    let solNfts: INFTMetadata[] | null = await fetchServerSolNfts(
      solanaAddress
    );
    // fetch eth nfts
    let ethNfts: INFTMetadata[] | null = await listNftsByAddress(
    ...
    // fetch near nfts
    let nearAddress = await getAddressForNetworkDb(
      kryptikWallet,
      nearNetworkDb
    );
    ...
    if (nearKryptikProvider.nearProvider) {
      try {
        let nearNfts = await listNearNftsByAddress(
          nearAddress,
    ...
    // fetch poaps
    let poapsList = await listPoapsByAddress(
      kryptikWallet.resolvedEthAccount.address
    );
    if (poapsList) {
  ...
  const fetchNftDataAccount = async function (
    account: string,
    networkTicker: string
  ) {
      ...
      <div className="flex flex-col md:flex-row pb-20">
        <div className="flex-1">
          ...
          <div className="flex flex-col mx-6">
            <div
              onClick={() => handleActiveCategoryChange(ActiveCategory.all)}
              className={`${
            ...
            <div
              onClick={() => handleActiveCategoryChange(ActiveCategory.eth)}
              className={`${
            ...
            >
              ...
              <div className={`ml-1 grow text-right`}>
                ...
                >
                  {nftList.filter((nft) => nft.networkTicker == "eth").length}
                </span>
              </div>
            </div>

            <div
              onClick={() => handleActiveCategoryChange(ActiveCategory.sol)}
              className={`${
            ...
            >
              ...
              <div className={`ml-1 grow text-right`}>
                ...
                >
                  {nftList.filter((nft) => nft.networkTicker == "sol").length}
                </span>
              </div>
            </div>

            <div
              onClick={() => handleActiveCategoryChange(ActiveCategory.near)}
              className={`${
            ...
            >
              ...
              <div className={`ml-1 grow text-right`}>
                ...
                >
                  {nftList.filter((nft) => nft.networkTicker == "near").length}
                </span>
              </div>
            </div>

            <div
              onClick={() => handleActiveCategoryChange(ActiveCategory.poaps)}
              className={`${
            ...
            >
              ...
              <div className={`ml-1 grow text-right`}>
                ...
                >
                  {nftList.filter((nft) => nft.isPoap).length}
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="flex-9 md:w-[80%]">
          <div className="flex flex-row">
            ...
            <div className="invisible md:visible dark:text-white min-h-[60px]">
              ...
              {activeCategory != ActiveCategory.poaps &&
                activeCategory != ActiveCategory.all &&
                activeCategoryNetworkDb && (
                  <div className="font-bold text-2xl ml-20 mb-6">
                    <img
                      className="w-10 mr-2 inline"
                      src={activeCategoryNetworkDb.iconPath}
                    />
                    <h1 className="inline">
                      {activeCategoryNetworkDb.fullName} Nfts
                    </h1>
                  </div>
                )}
            </div>
          ...
          {!isNFTFetched && (
            <div className="text-center content-center items-center place-items-center my-40 mr-4">
              ...
              <svg
                role="status"
                className="w-8 h-8 ml-6 text-white animate-spin text-center content-center inline"
                viewBox="0 0 100 101"
                fill="none"
                ...
                <div>
                  {
                    ...
                    isNFTFetched &&
                    routerParams &&
                    routerParams.networkTicker == activeCategory.toString() ? (
                      ...
                      <div className="text-3xl font-semibold text-lg dark:text-white text-center mt-40 mx-auto">
                        ...
                        <p className="text-sm text-slate-500 dark:text-slate-400">
                          <span className="italic">
                            {truncateAddress(
                              routerParams.account,
                              networkFromNetworkDb(routerParams.networkDb)
                            )}{" "}
                            is not an {routerParams.networkDb.fullName} address
                          </span>
                        </p>
                      </div>
                    ) : (
