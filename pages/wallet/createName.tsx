...
const CreateName: NextPage = () => {
  ...
  const [kryptikProvider, setKryptikProvider] = useState(
  ...
  const fetchNameData = async function () {
    ...
    let newResolvedAccount: IResolvedAccount | null = await resolveAccount(
      resolverParams
    );
    if (!newResolvedAccount) return;
    setResolvedAccount(newResolvedAccount);
    setCurrentAddress(newResolvedAccount.address);
    setReadableCurrentAddress(
      truncateAddress(newResolvedAccount.address, network)
    );
  ...
  const fetchNameAvailability = async function () {
    ...
    let accountIdToPassIn: string = name.includes(".near")
      ? name
      : name + ".near";
  ...
  const handleClickFinalizeName = async function () {
    ...
    // publish tx.
    let newTxPubdata: TransactionPublishedData | null =
      await reserveNearAccountName(publishParams);
    ...
    <div>
      ...
      <div className="max-w-lg mx-auto rounded-md border border-solid pt-4 pb-10 px-6 border-gray-400 dark:border-gray-700">
        <div className="flex flex-col">
          <div className="flex flex-row pb-4">
            ...
            <div className="text-sm dark:text-gray-700 text-gray-200 pl-2">
              <p>
                Still in development. Secondary token balances will not display.
              </p>
            </div>
          </div>
        ...
        <p className="text-slate-700 mb-2 dark:text-white">
          Create a unique Near name. Your custom name will be used for all
          actions on the NEAR Protocol blockchain.
        </p>

        {/* current near address and names */}
        {!isResolveLoading && (
          <div>
            <p className="mb-2 text-justify text-sm text-gray-500 dark:text-gray-400">
              Current name{resolvedAccount.names ? "s" : ""}:
            </p>
            <div className="grid grid-cols-3 gap-y-2">
              <span className="bg-sky-400 text-white font-semibold rounded-full py-1 px-2 mr-1">
                {readableCurrentAddress}
              </span>
              {resolvedAccount.names &&
                resolvedAccount.names.map((name: string) => (
                  <span
                    className="bg-sky-400 text-white font-semibold rounded-full py-1 px-2 ml-1 mr-1"
                    key={name}
                  >
                    {name}
                  </span>
        ...
        <div className="py-5 rounded mb-0 ">
          ...
          <input
            ...
            id="inline-full-name"
            placeholder={"myname.near"}
            value={name}
            onChange={(e) => handleUpdateNearName(e.target.value)}
          />
          {!isUpdateLoading && name.length >= 2 ? (
            <p
              className={`text-sm font-semibold ${
                nameIsAvailable ? `text-green-500` : `text-red-500`
              }`}
            >
              {nameIsAvailable ? "Woohoo! Your name is available." : updateMsg}
            </p>
          ) : (
        ...
        <div className="content-center mx-auto text-center">
          <button
            onClick={() => handleClickFinalizeName()}
            className={`bg-transparent text-sky-500 ${
              !nameIsAvailable
                ? "bg-gray-100 dark:bg-gray-900 hover:cursor-not-allowed hover:bg-gray-200 dark:bg-gray-800"
                : "hover:bg-sky-500"
            } font-semibold text-xl rounded-full hover:text-white py-2 px-14 mx-auto ${
              isUpdateLoading ? "hover:cursor-not-allowed" : ""
            } border border-sky-500 hover:border-transparent rounded-lg my-5 transition-colors duration-100`}
            disabled={isUpdateLoading || !nameIsAvailable}
          >
            Reserve My Name
