        ...
        <div className="bg-[#FBFDFD] dark:bg-gradient-to-br dark:from-[#0d0d0d] dark:to-[#0A0A0A] max-w-md mx-auto rounded-lg border border-solid dark:border-gray-800 hover:dark:border-green-400 border-gray-100 drop-shadow dark:text-white pt-2 px-2 pb-10">
            <div className="mt-2">
              <div className="flex flex-row mt-1">
                ...
                <h2 className="font-bold text-md ml-2 mb-1 text-transparent bg-clip-text bg-gradient-to-br from-blue-500 to-green-500">Kryptik</h2>
              </div>
              <div className='flex-grow'>
                <h1 className="text-3xl font-bold">Welcome</h1>
              </div>
              <div className="flex flex-col mt-2">
                <p className="text-gray-700 dark:text-gray-400 text-lg font-semibold">Create your secure, Web3 wallet in one click.</p>
                
                <div className="flex flex-col px-8 mt-4 mb-4">
                  <input type="email" className="bg-gray-200 dark:bg-gray-700 appearance-none border border-gray-200 rounded w-full py-3 px-4 text-gray-800 dark:text-white leading-tight focus:outline-none focus:bg-white focus:border-sky-400 dark:focus:border-sky-500 text-2xl" id="inline-full-name" placeholder="Enter your email" required onChange={(e) => setEmail(e.target.value)}/>
                  <button onClick={()=>handleLoginUser()} className={`bg-transparent hover:bg-green-500 text-green-500 text-2xl font-semibold hover:text-white py-2 px-4 ${isLoading?"hover:cursor-not-allowed":""} border border-green-500 hover:border-transparent rounded-lg mt-5 mb-2`} disabled={isLoading}>      
                                Create Wallet
                                {
                                        !isLoading?"":
                  ...
                  <p className="text-gray-400 dark:text-gray-500 text-sm">If you already have an account, you will be logged in.</p>
                </div>
                <div className="text-center max-w-2xl mx-auto content-center">
                 
                      <Link href="../wallet/import"><span className="text-sky-300 dark:text-sky-600 hover:cursor-pointer hover:text-blue transition-colors duration-300">or import existing seed</span></Link>
                  {
                    isLoading && 
