...
export interface IWeb3Service {
    ...
    createERC20Contract: (params: CreateEVMContractParameters) => Promise<Contract|null>;
    getProviderForNetwork(nw: NetworkDb): KryptikProvider;
    InternalStartService(): {};
    getSupportedNetworkDbs(): NetworkDb[];
    searchNetworkDbsAsync(searchQuery: string, onlySupported?: boolean): Promise<NetworkDb[]>;
    getAllNetworkDbs(onlySupported?: boolean): NetworkDb[];
    getKryptikProviderForNetworkDb(networkDb: NetworkDb): Promise<KryptikProvider>;
