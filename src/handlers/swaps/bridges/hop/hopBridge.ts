...
export async function BuildHopBridgeTransaction(
  params: IBuildHopBridgeParams
  ...
  const isSellOnLayerOne: boolean = isLayerOne(
  ...
  const accountNonce = await evmProvider.getTransactionCount(
    fromAccount,
    "latest"
  );
  const kryptikFeeData: TransactionFeeData = await getTransactionFeeDataEVM({
    tx: tx,
...
async function populateHopBridgeL1ToL2(
  params: IBuildHopBridgeParams
  ...
  // get token amount
  const tokenDecimals: number = sellTokenAndNetwork.tokenData
    ? sellTokenAndNetwork.tokenData.tokenDb.decimals
    : sellTokenAndNetwork.baseNetworkDb.decimals;
  let roundedAmountCrypto: number = roundToDecimals(tokenAmount, tokenDecimals);
  let amountToBridge: BigNumber = parseUnits(
    roundedAmountCrypto.toString(),
    tokenDecimals
  );
  // min amount to receive w/ max slippage
  const minAmountRounded: number = roundToDecimals(
    tokenAmount - tokenAmount * slippagePercentage,
    tokenDecimals
  );
  const amountOutMin: BigNumber = parseUnits(
    minAmountRounded.toString(),
    tokenDecimals
  );
  ...
  const tx: TransactionRequest =
    await hopBridgeContract.populateTransaction.sendToL2(...txOptions);
  return tx;
}
...
function getTotalFee(
  ...
  const isSourceChainLayerOne: boolean = isLayerOne(
    sellTokenAndNetwork.baseNetworkDb
  );
  const sellNetworkName = getHopNetworkName(
    sellTokenAndNetwork.baseNetworkDb.fullName
  );
  const buyNetworkName = getHopNetworkName(
    buyTokenAndNetwork.baseNetworkDb.fullName
  );
  const isDestinationRelayable: boolean =
    relayableChains.includes(buyNetworkName);
...
function getBonderAddress(
  ...
  const bonderAddress =
    allBonders[tokenTicker]?.[sellNetworkName]?.[buyNetworkName];
