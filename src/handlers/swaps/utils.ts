...
import {
  TokenAndNetwork,
  tokenAndNetworksAreEqual,
} from "../../services/models/token";

export function isSwapAvailable(
  ...
  if (tokenAndNetworksAreEqual(buyTokenAndNetwork, sellTokenAndNetwork))
    return false;
  // TODO: UPDATE TO SUPPORT NONEVM + CROSSCHAIN SWAPS
  if (
    buyTokenNetwork.networkFamily == NetworkFamily.EVM &&
    sellTokenNetwork.networkFamily == NetworkFamily.EVM &&
    // ensure same chain to and from
    buyTokenNetwork.fullName.toLowerCase() ==
      sellTokenNetwork.fullName.toLowerCase()
  ) {
    return true;
  }
  ...
  if (
    buyTokenNetwork.networkFamily == NetworkFamily.Solana &&
    sellTokenNetwork.networkFamily == NetworkFamily.Solana &&
    // ensure same chain to and from
    buyTokenNetwork.fullName.toLowerCase() ==
      sellTokenNetwork.fullName.toLowerCase() &&
    // ensure not a testnet
    !buyTokenNetworkDb.isTestnet &&
    !sellTokenNetworkDb.isTestnet
  ) {
    return true;
  }
  return false;
...
export function isValidHopBridge(
  buyTokenAndNetwork: TokenAndNetwork,
  sellTokenAndNetwork: TokenAndNetwork
): boolean {
  ...
  const isDestinationLayerTwo: boolean = isLayerTwo(
    buyTokenAndNetwork.baseNetworkDb
  );
  // eth->warapped eth on polygon
  if (
    sellTokenAndNetwork.baseNetworkDb.fullName.toLowerCase() == "ethereum" &&
  ...
  const isValidNetworkPair =
    sellTokenAndNetwork.baseNetworkDb.fullName.toLowerCase().trim() ==
      "ethereum" && isDestinationLayerTwo;
  // test for same token
  const isSameAsset: boolean = tokenAndNetworksAreEqual(
    buyTokenAndNetwork,
    sellTokenAndNetwork,
    false
  );
