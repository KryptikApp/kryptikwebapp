...
export async function BuildSwapTokenTransaction(
  swapParams: IBuildSwapParams
  ...
  const sellBaseNetwork: Network = networkFromNetworkDb(
    sellTokenAndNetwork.baseNetworkDb
  );
  const buyBaseNetwork: Network = networkFromNetworkDb(
  ...
  if (isValidHopBridge(buyTokenAndNetwork, sellTokenAndNetwork)) {
    ...
    let kryptikBridgeTx: KryptikTransaction | null =
      await BuildHopBridgeTransaction(swapParams);
    return kryptikBridgeTx;
  }
  if (
    buyBaseNetwork.networkFamily == NetworkFamily.EVM &&
    sellBaseNetwork.networkFamily == NetworkFamily.EVM
  ) {
    console.log("Routing swap to 0x swap builder");
    let kryptikEVMTx: KryptikTransaction | null = await BuildEVMSwapTransaction(
      swapParams
    );
    return kryptikEVMTx;
  }
  if (
    buyBaseNetwork.networkFamily == NetworkFamily.Solana &&
    sellBaseNetwork.networkFamily == NetworkFamily.Solana
  ) {
    console.log("Routing swap to Solana swap builder");
    let kryptikSolTx: KryptikTransaction | null = await BuildSolSwapTransaction(
      swapParams
    );
    return kryptikSolTx;
  }
