...
export async function getTransactionFeeDataSolana(
  params: IFeeDataSolParams
): Promise<TransactionFeeData> {
...
export function AggregateSolFees(txFees: TransactionFeeData[]) {
  ...
  for (const txFee of txFees) {
    ...
    masterFee.lowerBoundCrypto =
      masterFee.lowerBoundCrypto + txFee.lowerBoundCrypto;
    masterFee.upperBoundCrypto =
      masterFee.upperBoundCrypto + txFee.upperBoundCrypto;
