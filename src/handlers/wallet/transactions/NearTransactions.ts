...
export const signAndSendNEARTransaction = async function (
  params: ISignAndSendNearParameters
): Promise<TransactionPublishedData> {
  let txDoneData: TransactionPublishedData = { hash: "" };
  const { txNear, wallet, sendAccount, kryptikProvider } = params;
  if (!kryptikProvider.nearProvider) {
    throw new Error(
      `Error: Provider not set for ${kryptikProvider.networkDb.fullName}`
    );
  }
  let nearProvider = kryptikProvider.nearProvider;
  const serializedTx: Uint8Array = nearUtils.serialize.serialize(
    SCHEMA,
    txNear
  );
  ...
  const signature = await wallet.seedLoop.signTransaction(
    sendAccount,
    kryptikTxParams,
    kryptikProvider.network
  );
  // ensure signature was created
  if (!signature.nearFamilyTx) {
    throw new Error(
      `Error: Unable to create signature for ${kryptikProvider.networkDb.fullName} base layer transaction`
    );
  }
  ...
  const signedTransaction: SignedTransaction = new SignedTransaction({
    ...
    signature: new Signature({
      ...
      data: signature.nearFamilyTx,
    }),
  });
  // publish transaction to the blockchain
  try {
    const txPostResult: FinalExecutionOutcome =
      await nearProvider.connection.provider.sendTransaction(signedTransaction);
    console.log("NEAR POST RESULT");
    console.log(txPostResult);
    if (
      typeof txPostResult.status === "object" &&
      typeof txPostResult.status.Failure === "object"
    ) {
      throw new Error(
        `Error: NEAR transaction failed with error: ${txPostResult.status.Failure.error_message}`
      );
    }
...
export const BuildNEARTransfer = async function (
  params: NearTransactionParams
): Promise<KryptikTransaction | null> {
  const { tokenAndNetwork, tokenPriceUsd, kryptikProvider, txType } = params;
  if (!kryptikProvider.nearProvider) {
    return null;
  }
  ...
  if (txType == TxType.TransferToken) {
    if (!tokenAndNetwork.tokenData) {
      throw new Error(
        "Error: Token Data not provided for NEAR token transfer."
      );
    }
  ...
  let kryptikFeeData: TransactionFeeData = await getTransactionFeeDataNear({
...
export const createNearTransaction = async function (
  txIn: NearTransactionParams
): Promise<Transaction> {
  if (!txIn.kryptikProvider.nearProvider)
    throw new Error(
  ...
  let recentBlockhash: Buffer = nearUtils.serialize.base_decode(
    block.header.hash
  );
...
export const createNearTokenTransaction = async function (
  txIn: NearTransactionParams
) {
  if (!txIn.kryptikProvider.nearProvider)
    throw new Error(
  ...
  if (!txIn.contractAddress)
    throw new Error(
      "Error: No contract address provided for Near Token Transfer."
    );
  let nearProvider = txIn.kryptikProvider.nearProvider;
  // convert near amount to yocto units.. similar to wei for ethereum
  let amountToken: string = multByDecimals(
    txIn.valueNear,
    txIn.decimals
  ).asString;
  // arguments to pass in to function call
  let callArgs = { amount: amountToken, receiver_id: txIn.toAddress };
  // hardcoded deposit amount of 1 yocto
  // smallest nonzero amount possible
  let depositAmount: BN = numberToBN("1");
  let actions: Action[] = [];
  let storageAvailable = await isStorageBalanceAvailable(
    nearProvider,
    txIn.sendAccount,
...
const getStorageBalance = async function (
  nearProvider: Near,
  accountId: string,
  contractAddress: string
) {
  let nearAccount = await nearProvider.account(accountId);
  let storageBalance = await nearAccount.viewFunction(
    contractAddress,
    "storage_balance_of",
...
const isStorageBalanceAvailable = async function (
  nearProvider: Near,
  accountId: string,
  contractAddress: string
) {
