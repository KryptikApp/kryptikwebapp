...
export const signAndSendSOLTransaction = async function (
  params: ISignAndSendSolParameters
): Promise<TransactionPublishedData> {
  let txDoneData: TransactionPublishedData = { hash: "" };
  const { txs, wallet, sendAccount, kryptikProvider } = params;
  ...
  for (let txSol of txs) {
    currIndex += 1;
    console.log(
      `Signing and sending ${currIndex} of ${numTxToSign} Solana transactions.`
    );
    console.log(txSol);
    console.log("-------------");
    ...
    if (!txSol.verifySignatures()) {
      throw new Error(
        `Error: Unable to verify signature for ${network.fullName} transaction`
      );
    }
    ...
    // try to send tx
    try {
      const txPostResult = await solProvider.sendRawTransaction(
        txSol.serialize()
      );
      ...
      if (
        ...
        let minimumAmountSol: number = lamportsToSol(
          await solProvider.getMinimumBalanceForRentExemption(dataLength)
        );
        let msg = `Unable to publish ${
          kryptikProvider.networkDb.fullName
        } transaction. Note: Your Solana Account must hold at least ${roundCryptoAmount(
          minimumAmountSol
        )} ${formatTicker(
          kryptikProvider.networkDb.ticker
        )} to pay 'rent' for space used on the blockchain.`;
        throw new Error(msg);
      }
      throw e;
    }
...
export const createSolTransferTransaction = async function (
  txIn: SolTransactionParams
): Promise<KryptikTransaction> {
  if (!txIn.kryptikProvider.solProvider)
    throw new Error(
  ...
  let lastBlockHash = await txIn.kryptikProvider.solProvider.getLatestBlockhash(
    "finalized"
  );
...
export const createSolTokenTransferTransaction = async function (
  txIn: SolTransactionParams
): Promise<KryptikTransaction> {
  if (!txIn.kryptikProvider.solProvider)
    throw new Error(
  ...
  if (!txIn.tokenParamsSol)
    throw new Error(
      `No token data provided for ${txIn.tokenAndNetwork.baseNetworkDb.fullName}. Unable to create transaction.`
    );
  let transaction: Transaction = new Transaction();
  // destination token account for transfer
  let toPubkey = createEd25519PubKey(txIn.toAddress);
  let toTokenAccount = await createSolTokenAccount(
    txIn.toAddress,
    txIn.tokenParamsSol.contractAddress
  );
  // from token account for transfer
  let fromPubKey: PublicKey = new PublicKey(txIn.sendAccount);
  let fromTokenAccount = await createSolTokenAccount(
    txIn.sendAccount,
    txIn.tokenParamsSol.contractAddress
  );
  let mintPubKey = createEd25519PubKey(txIn.tokenParamsSol.contractAddress);
  // test if to account already exists
  const toAccountInfo = await txIn.kryptikProvider.solProvider.getAccountInfo(
    toTokenAccount
  );
  if (!toAccountInfo) {
  ...
  let amountToken: number = multByDecimals(
    txIn.valueSol,
    txIn.decimals
  ).asNumber;
