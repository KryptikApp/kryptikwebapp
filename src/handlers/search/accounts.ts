...
const accountOnclickFunction = function (params: IAccountClickHandlerParams) {
  ...
  router.push({
    pathname: "../gallery",
    query: { account: account, networkTicker: networkTicker, name: name },
  });
};

export const getAccountSearchSuggestions = async function (
  query: string,
  networkDb: NetworkDb
  ...
  if (network.networkFamily == NetworkFamily.EVM) {
    const ensSuggestions: ISearchResult[] = await getEnsSuggestions(
      query,
      networkDb
    );
  ...
  suggestions.push({
    resultString: query,
    onClickFunction: accountOnclickFunction,
    onClickParams: queryOnclickParams,
  });
  return suggestions;
};

const getEnsSuggestions = async function (
  query: string,
  networkDb: NetworkDb
): Promise<ISearchResult[]> {
  let suggestions: ISearchResult[] = [];
  try {
    ...
    for (const ensAccount of ensSuggestions) {
      let name =
        ensAccount.names && ensAccount.names[0] != ""
          ? ensAccount.names[0]
          : ensAccount.address;
      let onClickParams: IAccountClickHandlerParams = {
        account: ensAccount.address,
        name: ensAccount.names ? ensAccount.names[0] : undefined,
        networkTicker: networkDb.ticker,
      };
      suggestions.push({
        resultString: name,
        iconPath: ensAccount.avatarPath,
        onClickFunction: accountOnclickFunction,
