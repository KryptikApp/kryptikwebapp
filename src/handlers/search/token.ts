...
const tokenOnclickFunction = function (params: ITokenClickHandlerParams) {
  ...
  if (tokenTicker) {
    router.push({
      ...
      query: { networkTicker: networkTicker, tokenTicker: tokenTicker },
    });
  } else {
    router.push({
      pathname: "../coins/coinInfo",
      query: { networkTicker: networkTicker },
    });
  }
};

export const searchSuggestionsFromTokenAndNetworks = function (
  query: string,
  tokenAndNetworks: TokenAndNetwork[],
  clickHandler?: (selectedToken: TokenAndNetwork) => void,
  returnAll: boolean = false
): ISearchResult[] {
  let suggestions: ISearchResult[] = [];
  let newSuggestion: ISearchResult;
  const filteredTokens = returnAll
    ? tokenAndNetworks
    : filterTokenAndNetworkListByQuery(tokenAndNetworks, query);
  for (const token of filteredTokens) {
    if (clickHandler) {
      newSuggestion = {
        resultString: token.tokenData
          ? token.tokenData.tokenDb.name
          : token.baseNetworkDb.fullName,
        iconPath: token.tokenData
          ? token.tokenData.tokenDb.logoURI
          : token.baseNetworkDb.iconPath,
        iconPathSecondary: token.tokenData
          ? token.baseNetworkDb.iconPath
          : undefined,
      ...
      newSuggestion = {
        ...
        iconPathSecondary: token.tokenData
          ? token.baseNetworkDb.iconPath
          : undefined,
        onClickFunction: tokenOnclickFunction,
...
export const getTokenSearchSuggestions = function (
  ...
  for (const baseNetwork of networkList) {
    ...
    if (clickHandler) {
      newSuggestion = {
        resultString: baseNetwork.fullName,
        iconPath: baseNetwork.iconPath,
        onClickFunction: clickHandler,
        onClickParams: tokenAndNetwork,
      };
    } else {
      let queryOnClickParams: ITokenClickHandlerParams = {
  ...
  for (const token of tokenList) {
    ...
    for (const chainData of token.contracts) {
      ...
      let tokenAndNetwork: TokenAndNetwork = {
        ...
        tokenData: { tokenDb: token, selectedAddress: chainData.address },
      };
      // if swap validator is provided ensure... search result is a valid swap pair
      if (swapValidator) {
        if (!swapValidator.isValidSwapPair(tokenAndNetwork)) continue;
      ...
      if (clickHandler) {
        newSuggestion = {
          ...
          iconPathSecondary: baseNetwork.iconPath,
          onClickFunction: clickHandler,
          onClickParams: tokenAndNetwork,
        };
      } else {
...
export function filterTokenAndNetworkListByQuery(
  tokenAndNetworks: TokenAndNetwork[],
  query: string
): TokenAndNetwork[] {
...
export function filterTokenListByQuery(
  tokenList: TokenDb[],
  query: string
): TokenDb[] {
  query = query.toLowerCase();
...
export function filterNetworkListByQuery(
  networkList: NetworkDb[],
  query: string
): NetworkDb[] {
  query = query.toLowerCase();
  let filteredResult: NetworkDb[] = networkList.filter(
    (network) =>
      network.fullName.toLowerCase().includes(query) ||
      network.ticker.toLowerCase().includes(query)
  );
  return filteredResult;
}

export function searchTokenListByTicker(
  tokenList: TokenDb[],
  ticker: string
): TokenDb | null {
