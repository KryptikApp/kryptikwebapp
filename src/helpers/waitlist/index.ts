...
export const addEmailToWaitlist = async function (
  newEmail: string,
  errorHandler: (msg: string) => void
): Promise<number | null> {
  let isAlreadyAdded: boolean = await isEmailInWaitlist(newEmail);
  if (!isValidEmailAddress(newEmail))
    errorHandler(`${newEmail} is not a valid email.`);
  if (isAlreadyAdded) {
  ...
  await setDoc(
    doc(firestore, "waitlist", newEmail.toLowerCase()),
    waitlistData
  );
  let position: number | null = await getWaitlistPosition(newEmail);
  return position;
...
export const getWaitlistPosition = async function (
  newEmail: string
): Promise<number | null> {
...
export const isEmailInWaitlist = async function (
  email: string
): Promise<boolean> {
...
export const isOnAlphaTestList = async function (
  email: string
): Promise<boolean> {
