...
export function useKryptikAuth() {
  ...
  const ConnectWalletLocalandRemote = async function (
    ks: Web3Service,
    user: UserDB,
    seed?: string
  ): Promise<IWallet> {
    ...
    if (
      !UserExtraData.remoteShare ||
      kryptikConnectionObject.remoteShare != UserExtraData.remoteShare
    ) {
  ...
  const updateCurrentUserKryptik = async (user: UserDB) => {
    ...
    let extraUserDataUpdated: UserExtraData = {
      remoteShare: extraUserData.remoteShare,
      bio: user.bio,
      isTwoFactorAuth: extraUserData.isTwoFactorAuth,
    };
  ...
  const signInWithToken = async (
    ...
    let userCred: UserCredential = await signInWithCustomToken(
      firebaseAuth,
      customToken
    );
  ...
  const updateAuthContext = async (user: User, seed?: string, id?: string) => {
    ...
    if (seed != "") {
      newWalletKryptik = await ConnectWalletLocalandRemote(
        ks,
        formattedUser,
        seed
      );
    ...
    try {
      ...
      newWalletKryptik = updateWalletNetworks(
        newWalletKryptik,
        kryptikService,
        formattedUser,
        userExtraData
  ...
  const updateWalletNetworks = function (
    wallet: IWallet,
    ks: Web3Service,
    user: UserDB,
    extraUserData: UserExtraData
  ): IWallet {
  ...
  const writeExtraUserData = async function (
    user: UserDB,
    data: UserExtraData
  ) {
