...
export const resolveAccount = async function(params:IAccountResolverParams):Promise<IResolvedAccount|null>{
    ...
    switch(network.networkFamily){
        case(NetworkFamily.EVM):{
            let resolvedAccount:IResolvedAccount|null = await resolveEVMAccount(params);
            return resolvedAccount;
        }
        case(NetworkFamily.Near):{
            let resolvedAccount:IResolvedAccount|null = await resolveNEARAccount(params);
            return resolvedAccount;
        }
        case(NetworkFamily.Solana):{
            let resolvedAccount:IResolvedAccount|null = await resolveSOLAccount(params);
            return resolvedAccount;
        }
        default:{
            return null;
