...
export const fetchServerHistoricalPrices = async function(id: string,
    days:number):Promise<CoinMarketChartResponse|null>{
    // request balance data from api
    let dataResponse = await KryptikFetch('/api/prices', {method:"POST", timeout:10000, headers:{'Content-Type': 'application/json',}, body: JSON.stringify({id:id, days:days})})
