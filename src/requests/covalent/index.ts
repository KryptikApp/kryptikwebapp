...
export const getAssetsFromCovalent = async (
  ...
  try {
    ...
    console.error(
      `Error: Unable to fetch Covalent balance data for network with Covalent chain id: ${chainId}.`
    );
  }
  return null;
};
...
export const fetchServerBalances = async function (
  chainId: Number,
  accountAddress: string,
  currency: string
): Promise<CovalentAddressBalanceResponseData | null> {
  // request balance data from api
  let dataResponse = await KryptikFetch("/api/balances", {
    method: "POST",
    timeout: 4000,
