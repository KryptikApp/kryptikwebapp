...
const ListItemBalance: NextPage<Props> = (props) => {
  ...
  if (tokenAndNetwork.tokenData?.tokenBalance) {
    amount = roundCryptoAmount(
      Number(tokenAndNetwork.tokenData.tokenBalance.amountCrypto)
    );
    amountFiat = roundUsdAmount(
      Number(tokenAndNetwork.tokenData.tokenBalance.amountUSD)
    );
  } else {
    if (tokenAndNetwork.networkBalance) {
      amount = roundCryptoAmount(
        Number(tokenAndNetwork.networkBalance.amountCrypto)
      );
      amountFiat = roundUsdAmount(
        Number(tokenAndNetwork.networkBalance.amountUSD)
      );
    }
  }
  ...
  return (
    <div>
      {amount != null && amountFiat != null && (
        <li
          key={`${htmlKey}`}
          className="py-3 sm:py-4 hover:cursor-pointer rounded hover:gray-100 hover:dark:bg-[#141414]"
        ...
        >
          <div className="flex space-x-2 px-2">
            ...
            <div className="flex-grow text-right min-w-0">
              <p
                className={`text-lg font-medium text-gray-900 truncate dark:text-gray-200 ${
                  hideBalances ? "font-bold blur-sm" : ""
                }`}
              >
                ${amountFiat}
              </p>
