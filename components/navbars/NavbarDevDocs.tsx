...
export default function NavbarDevDocs() {
  ...
  async function fetchDevDocs() {
    ...
    let docsResponse = await KryptikFetch("/api/devDocs", {
      method: "POST",
      timeout: 8000,
  ...
  return (
    <nav className="">
      ...
      <div
        ...
        className={`${
          !showMenu && "hidden"
        } flex-col md:flex px-8 md:py-4 h-[92vh] md:w-[18vw] md:min-w-[250px] md:border-r-2 border-gray-200 dark:border-gray-800 text-black dark:text-white`}
      >
        {isLoadingDocs ? (
          <div className="text-md text-gray-600 dark:text-gray-300 max-w-lg mx-auto flex flex-row space-x-2">
            ...
            <svg
              role="status"
              className="w-3 h-3 animate-spin my-auto"
              viewBox="0 0 100 101"
              fill="none"
          ...
          <div className="flex flex-col space-y-4">
            {allDocs.length != 0 && (
              <DocDevCategoryPreview
                activeSlug={activeSlug}
                onDocSelection={handleDocSelection}
                alwaysOpen={true}
                docs={allDocs.filter(
                  (d) => d.category.toLowerCase().trim() == "getting started"
                )}
                categoryName="Getting Started"
                description="First steps when getting started with Kryptik."
              />
            )}
            {allDocs.length != 0 && (
              <DocDevCategoryPreview
                ...
                alwaysOpen={true}
                docs={allDocs.filter(
                  (d) => d.category.toLowerCase().trim() == "core concepts"
                )}
            ...
            {allDocs.length != 0 && (
              <DocDevCategoryPreview
                ...
                alwaysOpen={true}
                docs={allDocs.filter(
                  (d) => d.category.toLowerCase().trim() == "guides"
                )}
                categoryName="Guides"
                description="Simple wallet examples with code."
              />
            )}
          </div>
        )}
