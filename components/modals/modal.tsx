...
export default function Modal(props: Props) {
  ...
  const modalId = `${Math.random()}Modal`;
  const childrenId = `${Math.random()}Children`;
  ...
  return (
    <div>
      ...
      >
        ...
        <button
          type="button"
          className="md:hidden mb-2 mt-4 text-black bg-white rounded-full font-bold text-sm p-1.5 ml-auto items-center dark:bg-white dark:text-black transition ease-in-out hover:scale-110"
          onClick={() => onRequestClose()}
        >
          <svg
            className="w-5 h-5"
