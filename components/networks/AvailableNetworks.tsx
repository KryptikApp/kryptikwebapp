...
const AvailableNetworks: NextPage = () => {
  ...
  const fetchNetworks = async () => {
    ...
    if (supportedNetworks.length < maxAmountToShow) {
      ...
      let ethNetwork: NetworkDb | null =
        kryptikService.getNetworkDbByTicker("eth");
      let solNetwork: NetworkDb | null =
        kryptikService.getNetworkDbByTicker("sol");
      let nearNetwork: NetworkDb | null =
        kryptikService.getNetworkDbByTicker("near");
      let avaxcNetwork: NetworkDb | null =
        kryptikService.getNetworkDbByTicker("avaxc");
      if (ethNetwork) {
  ...
  return <IconSneakPeek icons={iconsToShow} groupTotal={allNetworks.length} />;
};
