...
const TxFee: NextPage<Props> = (props) => {
  ...
  return (
    <div>
      ...
      {txFeeData.isFresh && (
        <div>
          <p className={`${style ? style : "text-slate-400 text-sm"} inline`}>
            {feeLabelDisplay} $
            {txFeeData.lowerBoundUSD == txFeeData.upperBoundUSD
              ? txFeeData.upperBoundUSD < 0.01
                ? `<.01`
                : `${roundUsdAmount(txFeeData.upperBoundUSD, 2)}`
              : `${roundUsdAmount(txFeeData.lowerBoundUSD)}-$${roundUsdAmount(
                  txFeeData.upperBoundUSD
                )}`}
          </p>
      ...
      {!txFeeData.isFresh && (
        <div>
          <p className={`${style ? style : "text-slate-400 text-sm"} inline`}>
            Fees will be calculated on review
          </p>
          {/* spinner to show fees are updating */}
          {!feesLoaded && (
            <svg
