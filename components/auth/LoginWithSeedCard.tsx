...
const LoginWithSeedCard: NextPage = () => {
  ...
  const handleSeedChange = function (seedIn: string) {
    setSeed(seedIn);
  };
  ...
  async function handleCodeChage(newCode: string) {
    ...
    if (newCode.length == 7) {
      ...
      const approvedStatus: boolean = await signInWithToken(
        newCode,
        email,
        seed
      );
...
export default LoginWithSeedCard;
